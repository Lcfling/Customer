...
func init() {
	...
	message_creators[MSG_HTTP_MSG] = func() IMessage { return new(HttpSocket) }
	...
	message_descriptions[MSG_HTTP_MSG] = "MSG_HTTP_MSG"
	...
	external_messages[MSG_HTTP_MSG] = true
...
func (hs *HttpSocket) ToData() []byte {
	var l int64
	buffer := new(bytes.Buffer)
	l = int64(len(hs.url))
	binary.Write(buffer, binary.BigEndian, l)
	buffer.Write([]byte(hs.url))
	l = int64(len(hs.header))
	binary.Write(buffer, binary.BigEndian, l)
	buffer.Write([]byte(hs.header))
	l = int64(len(hs.body))
	binary.Write(buffer, binary.BigEndian, l)
	buffer.Write([]byte(hs.body))
	buf := buffer.Bytes()
	return buf
}

func (hs *HttpSocket) FromData(buff []byte) bool {
	if len(buff) < 24 {
		return false
	}
	var l int64
